...
class Runner(object):
  ...
  def __init__(self,
    ...
    self._sess = tf.Session('', config=tf.ConfigProto(allow_soft_placement=True, gpu_options=tf.GPUOptions(allow_growth=True)))
  ...
  def _initialize_checkpointer_and_maybe_resume(self, checkpoint_file_prefix):
    ...
    if latest_checkpoint_version >= 0 and self.testing:
  ...
  def _initialize_episode(self, run_mode_str):
    ...
    if not self.testing:
        if run_mode_str == 'eval' and not self.sticky_actions:
            noops = np.random.randint(1, 31)
            for _ in range(noops):
                #print ('noop')
                initial_observation, _, done, _ = self._environment.step(0)
                if done:
                    initial_observation = self._environment.reset()
  ...
  def _run_one_episode(self, run_mode_str):
    ...
    action = self._initialize_episode(run_mode_str)
    is_terminal = False
